...
//produtor consumidor --> 
public class Server {
	protected GenericConsumer<Socket> consumer;		//esta em todos os brockers servidores
	...
	public void begin(){
		try{
			...
			//log write mechanism
			consumer = new PubSubConsumer<Socket>(resource);		//uma thread comsumidora para garantir ordem
			
			consumer.start();	//thread paralela (altamente acoplada com o pubsub)
	...
	protected void listen(){
        ...
        while(! resource.isStopped()){
            ...
            try {
            	Socket clientSocket = this.serverSocket.accept();			//conecta com algum brocker
                
            	resource.putRegister(clientSocket);	//coloca o socket numa lista
            } catch (IOException e) {
